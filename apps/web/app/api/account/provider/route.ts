...
export async function GET() {
  ...
  const response = await fetch(`${apiBase}/v1/account/provider`, { headers: { authorization: `Bearer ${auth.accessToken}` }, cache: 'no-store' });
...
export async function POST(request: Request) {
  ...
  const response = await fetch(`${apiBase}/v1/account/provider/onboard`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${auth.accessToken}` }, body: JSON.stringify(await request.json()), cache: 'no-store' });
  return NextResponse.json(await response.json(), { status: response.status });
}
