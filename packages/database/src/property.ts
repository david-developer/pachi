...
export class PropertyDraftStore {
  ...
  public async drafts(userId: string): Promise<ListingDraft[]> {
    const provider = await this.requireDraftProvider(userId);
    const rows = await this.client<ListingRow[]>`SELECT l.id, l.property_id, l.provider_account_id, l.purpose, r.id AS revision_id, r.version, r.title, r.description, o.id AS offering_id, ov.id AS offering_version_id, ov.currency, ov.amount_minor, ov.pricing_period, ov.negotiable, ov.deposit_amount_minor, ov.advance_months, ov.minimum_lease_months, ov.utilities_included, ov.service_charge_amount_minor, ov.weekly_amount_minor, ov.minimum_nights, ov.guest_limit, ov.check_in_time::text, ov.check_out_time::text, ov.cleaning_fee_minor, ov.available_from::text FROM listings l JOIN listing_revisions r ON r.id = l.current_revision_id JOIN offerings o ON o.listing_id = l.id JOIN offering_versions ov ON ov.id = o.current_version_id WHERE l.provider_account_id = ${provider.account_id} AND l.publication_status = 'DRAFT' ORDER BY l.updated_at DESC`;
  ...
  private async readDraft(transaction: postgres.Sql, listingId: string, providerAccountId: string): Promise<ListingDraft> {
    const rows = await transaction<ListingRow[]>`SELECT l.id, l.property_id, l.provider_account_id, l.purpose, r.id AS revision_id, r.version, r.title, r.description, o.id AS offering_id, ov.id AS offering_version_id, ov.currency, ov.amount_minor, ov.pricing_period, ov.negotiable, ov.deposit_amount_minor, ov.advance_months, ov.minimum_lease_months, ov.utilities_included, ov.service_charge_amount_minor, ov.weekly_amount_minor, ov.minimum_nights, ov.guest_limit, ov.check_in_time::text, ov.check_out_time::text, ov.cleaning_fee_minor, ov.available_from::text FROM listings l JOIN listing_revisions r ON r.id = l.current_revision_id JOIN offerings o ON o.listing_id = l.id JOIN offering_versions ov ON ov.id = o.current_version_id WHERE l.id = ${listingId} AND l.provider_account_id = ${providerAccountId}`;
...
type ListingRow = { id: string; property_id: string; provider_account_id: string; purpose: ListingPurpose; revision_id: string; version: number; title: string | null; description: string | null; offering_id: string; offering_version_id: string; currency: 'XAF'; amount_minor: number | string | null; pricing_period: 'MONTHLY' | 'TOTAL' | 'NIGHTLY'; negotiable: boolean; deposit_amount_minor: number | string | null; advance_months: number | null; minimum_lease_months: number | null; utilities_included: boolean | null; service_charge_amount_minor: number | string | null; weekly_amount_minor: number | string | null; minimum_nights: number | null; guest_limit: number | null; check_in_time: string | null; check_out_time: string | null; cleaning_fee_minor: number | string | null; available_from: string | null };
